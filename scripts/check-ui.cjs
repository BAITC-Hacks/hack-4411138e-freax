...
(async () => {
  ...
  try {
    ...
    await page.locator('#before-list .file-name').waitFor();
    const filename=await page.locator('#before-list .file-name').innerText();
    for(const lang of ['kk','en','ru']){
      await page.locator(`[data-lang="${lang}"]`).click();
      assert.equal(await page.locator('html').getAttribute('lang'),lang);
      assert.equal(await page.locator('#before-list .file-name').innerText(),filename);
    }
    ...
    await page.locator('#finding-search').fill('ДИТААД');
    await page.locator('[data-lang="kk"]').click();
    assert.equal(await page.locator('#finding-search').inputValue(),'ДИТААД');
    ...
    await page.locator('#left-document').selectOption('before');
    ...
    assert.match(await page.locator('#left-notice').innerText(),/не подтверждён/);
