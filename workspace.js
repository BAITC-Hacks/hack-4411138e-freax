...
export function createWorkspace(hooks){
 ...
 $('chat-source-ask').addEventListener('click',()=>{const ref=active?.source;if(!ref)return;if(active.kind!=='demo'){const selected=resolveRef(active,ref);closeSource();showTab('results');hooks.askFragment(selected.doc,selected.fragment);return;}closeSource();addContext(ref);});
 ...
 document.addEventListener('click',e=>{
  ...
  if(el.dataset.askFragment&&active.kind!=='demo'){const selected=resolveRef(active,refFor(el.dataset.askFragment));showTab('results');hooks.askFragment(selected.doc,selected.fragment);return;}
  ...
  if(el.dataset.artifactDownload){const a=active.artifacts.find(a=>a.id===el.dataset.artifactDownload);if(a)hooks.download(new Blob([a.content],{type:'text/markdown;charset=utf-8'}),`teledoc-${active.kind==='demo'?'DEMO-C010':'case'}-conclusion-v${a.version}-${a.locale}.md`);return;}
 ...
 $('case-export').addEventListener('click',()=>{if(active)hooks.download(new Blob([hooks.markdown(active.snapshot)],{type:'text/markdown;charset=utf-8'}),`teledoc-${active.kind==='demo'?'DEMO-C010':'case'}-analysis-v${active.analysisVersion}-${language}.md`);});
