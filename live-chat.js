...
export function createLiveChat(root,getConfig){
 ...
 const dialog=document.createElement('dialog');dialog.className='settings-dialog live-source';document.body.append(dialog);
 ...
 async function send(retry=false){
  ...
  try{await api(path+'/chat/messages',pending,45000);if(token!==generation)return;pending=null;draft='';findingIds=[];await refresh();}
 ...
 root.addEventListener('input',e=>{if(e.target.id==='live-chat-input'){draft=e.target.value;const button=root.querySelector('button[type=submit]');button.disabled=!canSend()||!draft.trim();}});
 ...
 return {set(value){if(snapshot?.analysis_id===value?.analysis_id&&snapshot===value){render();return;}generation++;clearTimeout(timer);if(dialog.open)dialog.close();snapshot=value;history=null;loading=false;sending=false;findingIds=[];draft='';error='';pending=null;render();refresh();},locale:render,
