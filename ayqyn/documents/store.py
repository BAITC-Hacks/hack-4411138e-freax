...
import json
import re
from ayqyn.documents.text import normalized
...
class DocumentStore:
    ...
    def _context(self, fragment_id, scope):
    ...
    def _read_context(self, fragment_id, scope, tool):
        result=self._context(fragment_id,scope)
        arguments={'fragment_id':fragment_id}
        if tool == 'read_context':
            arguments['scope']=scope
        return self._return_read(tool, arguments, result)

    def read_context_page(self, fragment_id, scope='section', offset=0, *, limit_chars=18000):
        """Bounded native-tool view; full fragments only, source-order pagination.

        The archive retains structural metadata. Model-visible blocks carry
        context roles instead of repeatedly serializing every relationship ID.
        Only blocks actually delivered count toward read coverage.
        """
        if type(offset) is not int or offset<0:
            raise ValueError('offset must be a nonnegative integer.')
        context=self._context(fragment_id,scope)
        blocks=context['fragments']
        ...
        result['notice']='Returned blocks only; previous/later pages are omitted. Continue with next_offset; truncation is not absence.'
        ...
        for p in blocks[offset:]:
            ...
            # Reserve space for pagination metadata, never slice a quotation.
            if len(json.dumps(result,ensure_ascii=False))+128>limit_chars:
                result['fragments'].pop()
                if not result['fragments']:
                    raise ContextLimitError(len(json.dumps(block,ensure_ascii=False)),limit_chars)
                break
        end=offset+len(result['fragments'])
        result.update(next_offset=end if end<len(blocks) else None,
                      omitted_fragments=len(blocks)-len(result['fragments']),
                      truncated=offset>0 or end<len(blocks))
        result=deepcopy(result)
        self.read_ids.update(p['id'] for p in result['fragments'])
        self.actions.append({'tool':'read_context','arguments':{'fragment_id':fragment_id,'scope':scope,'offset':offset},'result':deepcopy(result)})
        return result

    def coverage_summary(self):
        coverage=self.get_coverage()
        return {'documents':[{k:d[k] for k in ['id','name','role','read_status','indexed_fragments','read_fragments','fully_read','error','warnings','unread_pages']}
                             for d in coverage['documents']],
                'after_fully_read':coverage['after_fully_read'],'errors':coverage['errors'],
                'warnings':coverage['warnings'],'limitation':coverage['limitation'],
                'search_count':len(coverage['search_scopes'])}
    ...
    def owner_evidence_candidates(self, evidence, owner):
        """Only the cited block and its actual ancestors, never nearby headings."""
        found={}
        for item in evidence if isinstance(evidence,list) else []:
            key=item.get('fragment_id') if isinstance(item,dict) else None
            seed=key
            while key in self.fragments:
                block=self.fragments[key]
                if normalized(owner) in normalized(block['text']) and key in self.read_ids:
                    found[key]={'fragment_id':key,'linked_from':seed,'was_read':True,
                                'relation':'same_block' if key==seed else 'structural_ancestor'}
                    if len(block['text'])<=3000:found[key]['quote']=block['text']
