...
class EmbeddingTests(unittest.TestCase):
    def test_shared_transport_default_dimensions_and_dynamic_deadline(self):
        ...
        with patch('ayqyn.providers.embeddings.llm.request_json_api', return_value=response([[1.0] * 512])) as request:
    ...
    def test_legacy_model_omits_unsupported_dimensions_and_orders_response_indexes(self):
        ...
        with patch('ayqyn.providers.embeddings.llm.request_json_api', return_value=payload) as request:
    ...
    def test_provider_identity_pinned_while_credentials_remain_dynamic(self):
        ...
        with patch('ayqyn.providers.embeddings.llm.request_json_api', return_value=response([[1]], 'custom')) as request:
            embedder(['sample'])
    ...
    def test_input_and_deadline_bounds_fail_before_provider_call(self):
        with patch('ayqyn.providers.embeddings.llm.request_json_api') as request:
    ...
    def test_response_rejects_count_indexes_model_and_dimensions(self):
        ...
        for payload in payloads:
            with self.subTest(payload=str(payload)[:70]), patch('ayqyn.providers.embeddings.llm.request_json_api', return_value=payload):
                with self.assertRaises(ValueError):
                    OpenAIEmbedder({})(['sample'])
        payload = response([[1] * 512, [1] * 512])
        payload['data'][1]['index'] = 0
        with patch('ayqyn.providers.embeddings.llm.request_json_api', return_value=payload), self.assertRaises(ValueError):
